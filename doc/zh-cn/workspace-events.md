...
**绘制事件（刷新时触发，不是点击）**
- **LV_EVENT_DRAW_MAIN**：绘制控件主体时触发
- **LV_EVENT_DRAW_POST**：子控件绘制完成后触发，适合叠加层
- **LV_EVENT_DRAW_TASK_ADDED**：创建 draw task 时触发（自动开启 `SEND_DRAW_TASK_EVENTS`）

绘制事件的动作只能选 **自定义代码** 或 **调用函数**。回调运行在渲染期间，禁止修改控件属性/样式，也禁止创建或删除控件。设计器画布不会模拟这些事件，请通过编译/模拟器验证。
...
- 在 `custom_func.c` 中实现该函数（详见 **[自定义代码](workspace-custom-code.md)**）

#### 自定义代码参数

动作类型选择 **「自定义代码」** 时，可配置：

- **#include 和变量**：写入对应页面 `.c` 文件顶部
- **功能实现**：内联写入该事件的回调函数

详细说明请参阅 **[自定义代码](workspace-custom-code.md)**。
...
- **[自定义代码](workspace-custom-code.md)** - 事件/页面自定义代码与 C 扩展
