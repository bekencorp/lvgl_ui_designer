...
- 新增**时间轴动画**功能：支持基于关键帧编辑组件属性动画，画布中可即时预览（支持批量预览与一键停止）
- 新增动画事件类型：在事件动作中可触发组件动画播放
- 新增组件动画属性面板与 Animation 编辑器入口，配置流程更顺畅

#### 优化

- 应用启动新增单实例锁，防止重复打开多个 LVGL UI Designer 实例
- 工作台终端新增杀毒软件白名单引导：当检测到工具被杀毒软件拦截时，提供一键查看处理指引

#### Bug修复

- 修复字体资源中"自定义字符"同步时，TabView 标签页（tabs）的标题文本未被同步收集的问题
- 修复多次复制粘贴组件时，组件树 UI 显示错乱的问题
- 修复字体生成在大字符集下偶发超时的问题
- 修复部分场景下 i18n 词条导入异常的问题
- 修复多个组件引用同一张图片资源时，并行转换出现 "Retrying image conversion" 警告的问题
- 修复部分场景下图片输出目录不存在导致 C 代码生成失败的问题
...
- Win组件支持添加子组件
- Tileview组件支持添加多个Tile页面，每个Tile页面支持添加子组件
- Menu组件支持支持添加多个菜单项，每个菜单项支持添加子组件
- Tabview组件支持添加多个Tab栏，每个Tab页面支持添加子组件

#### Bug修复

- 解决Arc组件Knob样式设置背景图片，C编译报错的问题

### 1.1.11

#### 功能

- List、msgbox、tabview、menu、win、dropdown组件新增多语言功能
- 新增字体删除功能

#### 优化

- 翻译预览现修改为跟随默认语言改动
- animimg组件可以看到所添加图片的文件名

#### Bug修复

- 修复属性面板样式概率显示错乱问题
- 修复自定义组件迁移/补全时，把用户关掉的样式分组又自动打开了的数据一致性问题 
- 修复自定义组件保存时，资源依赖未复制的问题

### 2.0.1

#### 功能

- 🚀新增在AI编辑器（Curosr,TRAE）中通过对话方式来生成、修改项目中的UI。
...
- AI设计新增 TRAE CN 和 Codex 支持
- AI设计新增对事件、动画、多语言功能的支持
- 新增项目导出设置，可自定义beken_generated目录导出位置
- 组件添加旋转属性
- 导出设置中新增字体抗锯齿精度设置（仅支持C语言编译效果）

#### 优化

- 优化AI设计的速度和流畅性
- 优化文档中链接颜色，修改部分文档描述错误
- 组件树面板最大宽度增大，方便查看组件名称
- Textarea组件样式中新增内边距样式组

#### Bug修复

- 修复使用TRAE进行AI设计时无法正确调用MCP的问题
- 修复Flex布局中子组件实际位置与选择框不对应的问题
...
- 修复文本框最大输入长度设置后导出代码未生效的问题
- 修复页面切换动画时长设置为0时导出代码中仍显示300的问题
- 修复tabview组件修改组件名称后，导出代码中名称错误的问题
- 修复主题颜色列表无法滚动查看的问题
...
- 优化自定义组件加入画布中后的名称规则
- 组件最大嵌套层数增加到20
- 页面数据错误导致渲染失败时给出提示，并支持修复数据
- 优化部分AI设计组件效果


#### Bug修复

- 修复画布内选中组件时组件树中对应组件未出现选中状态的问题
- 修复容器内拖入子组件时容器边框消失的问题
- 修复按钮字体属性对齐无效的问题
- 修复micropython预览启动页设置无效的问题
...
- 字体导出新增动态 TTF支持，可在项目设置->字体生成中选择点阵字或动态 TTF（tiny_ttf），导出字体可内置固件或存到 SD 卡、Flash
- 图片外部存储新增 Flash存储
- 图片资源支持“导出”，未被画布引用的图片也可按原始或自定义尺寸、与属性面板一致的颜色格式生成内嵌 C 数组和声明，供自定义代码调用
- 事件系统新增绘制事件：`LV_EVENT_DRAW_MAIN`、`LV_EVENT_DRAW_POST`、`LV_EVENT_DRAW_TASK_ADDED`（刷新管线触发，非点击）

#### 优化

- 动画时间轴关键帧支持拖动调整时间点
- 导出的 C 代码兼容 LVGL 9.3 和 9.5

#### Bug修复

- 修复关键帧动画中颜色动画预览与模拟器中不一致的颜色不一致问题
- 修复MicroPython模拟器中页面加载事件无效的问题
- 修复画布和 MicroPython 预览中，容器有子控件时圆角 1px 边框显示不对的问题
- 修复 Flex 布局下嵌套子组件选择框与实际位置错位、无法拖动的问题
- 修复自定义字体在预览与 C 模拟器中文字位置不一致的问题
...
- 支持导出项目中未被引用的图片，可配置内嵌、SD 卡或 Flash 存储及 PNG/BIN 格式
- 支持修改工程名称
- 支持复制工程
- 支持从首页卡片中移除工程